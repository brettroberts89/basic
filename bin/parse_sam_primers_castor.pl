...
my $scaffolds = "/Users/robotien/Desktop/Brett/SG_Biofuels/dilara/linkage_map/castor_scaff.csv";
...
print OUT "Contig,Contig_length,F_primer_pos,R_primer_pos,Orientation,Linkage_group,Marker_name,Pos_cM,Marker_type,Insert_size,Amplicon_size,F_primer_CIGAR,R_primer_CIGAR,Multimapping,Type\n";
while (my $line = <SAM>) {
    next if ($line =~ /^\@/);
...
foreach my $paired_seq (keys %mates) {
    ...
    if ($scaffold_mate eq $scaffold) {
        ...
        print "$scaffold_mate,$scaffold_length,$pos_mate,$pos\n";
