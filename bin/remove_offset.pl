#TITLE: remove_offset.pl
#AUTHOR: BRETT ROBERTS
#DATE LAST MODIFIED: 08/22/2012
#PURPOSE: maps SNPs from all.mergedSNPs.vcf to contigs and determines position relative to start of contig
...
while (my $line = <VCF>) {
    chomp $line;
    if ($line =~ /^1/) {                        #if the line begins with a 1, which is the case for everything but the header
        my @line = split(/\D/, $line);          #split the line into an array at any value that is not a digit
